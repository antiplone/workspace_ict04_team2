...
<body id="location_main_body">
		<!-- 이미지 클릭 시 '구' 선택 팝업창 => 선택 완료 시 해당 '구'에 맞는 리스트 페이지로 이동 -->
		<div class="main_select" style='background-color: #e1ecf7'>
			<form id="locationMain" name="locationMain" action="#" method="post">
				<table style="margin-left: auto; margin-right: auto;">
					<div class="container">
						<tr>
							<td class="si_title">	<!-- 전국 선택 시 별도 구 선택 없이 모든 지역 리스트 보임 -->
								<a style="display: inline-block;width:150px" onclick="homeMove('${path}/location_main.lc')">
						...
						<tr>
							<td colspan="5" align="center">
								...
								<c:if test="${paging.startPage > 5}">
									<a onclick="homeMove('${path}/location_main.lc?pageNum=${paging.prev}')"><input type="button" value="<<" class="page_btn"></a>
								</c:if>
								
								<!-- 페이지 번호 처리 -->
								<c:forEach var="num" begin="${paging.startPage}" end="${paging.endPage}">
									<a onclick="homeMove('${path}/location_main.lc?pageNum=${num}')"><input type="button" value="${num}" class="page_btn"></a>
								</c:forEach>
								 
								<!-- 다음 버튼 활성화 -->
								<c:if test="${paging.endPage < paging.pageCount}">
									<a onclick="homeMove('${path}/location_main.lc?pageNum=${paging.next}')"><input type="button" value=">>" class="page_btn"></a>
								</c:if>
							</td>
						</tr>
					</div>
